...
class ReporteIva():
    ...
    def __init__(self):
        # Get the uid
        self.sock_common = xmlrpclib.ServerProxy ('http://192.168.0.90:8070/xmlrpc/common')
        self.uid = self.sock_common.login(bd, user, pwd)

        #replace localhost with the address of the server
        self.sock = xmlrpclib.ServerProxy('http://192.168.0.90:8070/xmlrpc/object')

    def recuperar_partner(self,id_partner):
        args_partner = [('id', '=', id_partner),('customer','=','true')]
 ...
 #           print('hola')
    ...
    def venta(self):
        ...
        args = [('date_invoice', '>=', '01/01/2015'),('date_invoice', '<=', '31/01/2015'),('state','=','open')] #query clause
        ...
        reporte_iva_final = open("/home/alan/Escritorio/reporte_iva_venta.ods","a")
        reporte_iva_final.write(('%s,%s,%s,%s,%s,%s,%s,%s,%s') %("Fecha Factura","Numero Factura","Cliente","Cuit/Dni","Total","Iva 21%","Iva 10%","Neto","\n"))
